...
def get_parser(fixed_arguments: List[str] = []):
    ...
    parser.add_argument('--attribution_layer_name', default='previous')
...
if args.embeddings == "pure":
    ...
    figure_title = f"PURE: {attribution_layer_name}"
    figure_file_name = f"neurons_{'_'.join([str(n.item()) for n in neurons])}_{args.embeddings}_{attribution_layer_name}.pdf"
    ...
    figure_title = args.embeddings
    figure_file_name = f"neurons_{'_'.join([str(n.item()) for n in neurons])}_{args.embeddings}.pdf"
...
for inds_ in [neurons]:
    ...
    plt.savefig(f"{path}/{figure_file_name}", dpi=300)
