...
def get_parser(fixed_arguments: List[str] = []):
    ...
    # you investigate mixed-selectivity of neurons in this layer.  
    parser.add_argument('--layer_name', type=str, default="block_3")
    
    # you compute relevance scores in this layer. 
    # By default, yo check the immidiate previous layer
    parser.add_argument('--attribution_layer_name', type=str, default='previous') 
...
attribution_layer_name = args.attribution_layer_name
# at which layer you will check relevance scores.
if attribution_layer_name == 'previous': # relevance in the immediate previous layer 
...
#     DINO_embeddings = torch.cat(DINO_embeddings, dim=0)
    ...
    print(f"REL Mean distance inside clusters ({attribution_layer_name}): {np.mean(inner_distances)}")
    print(f"REL Mean distance inbetween clusters({attribution_layer_name}): {np.mean(inter_distances)}")
...
print(f"REL Intra mean distance ({attribution_layer_name}): {np.mean(inner_rel)}")
print(f"REL Inter mean distance ({attribution_layer_name}): {np.mean(inter_rel)}")
...
torch.save(vals_full, f"results/interpretability/{dataset_name}/{model_name}/interpretability_{n_clusters}clusters_{layer_name}_{SPLIT}_{attribution_layer_name}.pt")
...
# x = ["CLIP", "DINO", f"PURE ({attribution_layer_name})", "Activation"]
